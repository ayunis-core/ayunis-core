...
const ROUTE_MAPPINGS = [
  ...
  },
  {
    ...
    files: ['ayunis-core-frontend/src/app/routes/_authenticated/settings.account.tsx'],
...
function mappedRoutesFor(file) {
  for (const mapping of ROUTE_MAPPINGS) {
    if (mapping.files?.includes(file) || hasPrefix(file, mapping.prefixes ?? [])) {
...
function isFrontendChange(file) {
  return file.startsWith('ayunis-core-frontend/') || file.startsWith('packages/ui/');
}
...
export function selectScreenshotRoutes(files) {
  ...
  for (const file of frontendFiles) {
    ...
    if (isGlobalFrontendChange(file) || file.startsWith('ayunis-core-frontend/')) {
      return { frontendChanged: true, routes: ALL_SCREENSHOT_ROUTES };
    }
  }

  return {
    frontendChanged: true,
    routes: ALL_SCREENSHOT_ROUTES.filter((route) => selectedRoutes.has(route)),
  };
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const result = selectScreenshotRoutes(readFilesFromArgs(process.argv.slice(2)));
