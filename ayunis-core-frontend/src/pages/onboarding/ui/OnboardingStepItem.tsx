...
export default function OnboardingStepItem({
  ...
  const handleAction = () => {
    ...
    // route (like the rest of the app), but store the resolved pathname for
    // pending-step matching against location.pathname.
    if (isAddDocumentsStep && firstKnowledgeBase) {
      setPendingStep(
        step.id,
        `/knowledge-bases/${firstKnowledgeBase.id}`,
        origin,
      );
      void navigate({
