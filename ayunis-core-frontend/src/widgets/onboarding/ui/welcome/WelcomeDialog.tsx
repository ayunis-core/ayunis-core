...
import Step3Assistants from "./steps/Step3Assistants";
...
export default function OnboardingDialog() {
  ...
  const storageKey = useMemo(() => {
    return `${STORAGE_KEY_PREFIX}:${user?.email ?? "anon"}`;
  }, [user?.email]);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen} >
      <DialogContent
        ...
        className="sm:max-w-5xl min-h-[552px] p-0 overflow-hidden border-0"
      >
        {/* p-0 border-0 bg-transparent shadow-none overflow-hidden */}
        {step === 1 && <Step1Welcome onNext={goNext} />}
        {step === 2 && <Step2ChatIntro onNext={goNext} />}
        {step === 3 && <Step3Assistants onBack={goBack} onNext={goNext} />}
        {step === 4 && <Step4Invite onBack={goBack} onNext={goNext} />}
        {step === 5 && <Step5Complete onNext={goNext} />}
      </DialogContent>
    </Dialog>
  );
}
