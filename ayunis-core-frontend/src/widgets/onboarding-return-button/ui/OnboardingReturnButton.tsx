...
export function OnboardingReturnButton() {
  ...
  // Persistent shortcut back to onboarding from anywhere in the app. Shown
  // whenever onboarding isn't hidden — except on the onboarding page itself,
  // while a tour spotlight is active, or before we know the hidden flag (so a
  // user who hid onboarding doesn't get a flash of the pill on first load).
  const show =
    !isLoading &&
    !hidden &&
    !isTourActive &&
