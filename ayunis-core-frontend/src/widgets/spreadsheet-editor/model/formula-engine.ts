...
function assertLocalSheet(sheet: string | undefined): void {
  if (sheet !== undefined && sheet.toUpperCase() !== SHEET.toUpperCase()) {
    throw new FormulaError('#REF!');
  }
}

function resolveFormulaCell(ref: CellRef): Evaluated {
  assertLocalSheet(ref.sheet);
  return resolveCell(ref.row, ref.col - 1);
}
...
const parser = new FormulaParser({
  onCell: (ref) => resolveFormulaCell(ref).value,
  onRange: (ref) => {
    assertLocalSheet(ref.sheet);
    ...
    const maxRow = context ? context.state.rows.length + 1 : 0;
    const maxCol = context ? context.state.columns.length : 0;
    for (
