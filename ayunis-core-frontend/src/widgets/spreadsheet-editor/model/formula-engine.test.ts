...
describe('computeDisplayValues', () => {
  ...
  it('rejects references to unsupported sheets', () => {
    const display = computeDisplayValues(
      grid(['Amount', 'Total'], [['10', '=OtherSheet!A2']]),
    );

    expect(display[0][1]).toBe('#REF!');
  });

  it('rejects ranges on unsupported sheets', () => {
    const display = computeDisplayValues(
      grid(['Amount', 'Total'], [['10', '=SUM(OtherSheet!A2:A3)']]),
    );
  ...
  it('preserves hash-prefixed text returned by formulas', () => {
    const display = computeDisplayValues(
      grid(['A', 'B'], [['="#1 seller"', '=A2&""']]),
    );

    expect(display[0][0]).toBe('#1 seller');
    expect(display[0][1]).toBe('#1 seller');
  });

  it('evaluates reverse ranges in ascending sheet order', () => {
    const display = computeDisplayValues(
      grid(
        ['A', 'B'],
        [
