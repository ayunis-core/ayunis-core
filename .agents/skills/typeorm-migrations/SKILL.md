...
Change the schema definition in the relevant record file. If the project has a `BaseRecord`, extend it so you get `id`, `createdAt`, `updatedAt` automatically:
...
**Prerequisites:** The dev stack must be running so that Postgres is available on localhost.
