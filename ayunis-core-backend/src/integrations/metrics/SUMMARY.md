...
- `metrics.module.ts` — NestJS module wiring. Registers `PrometheusModule` at the `/metrics` path with a custom controller, applies `MetricsAuthMiddleware` to protect it, and registers `PrometheusMetricsListener` as a provider.
...
- `listeners/prometheus-metrics.listener.ts` — `PrometheusMetricsListener` subscribes to domain events (`UserCreatedEvent`, `UserMessageCreatedEvent`, `AssistantMessageCreatedEvent`, `RunExecutedEvent`, `InferenceCompletedEvent`, `TokensConsumedEvent`, `ToolUsedEvent`, `ThreadMessageAddedEvent`) and records the corresponding Prometheus metrics. This consolidates all metric recording that previously lived in individual use cases and services.
