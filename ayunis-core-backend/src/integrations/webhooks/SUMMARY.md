...
- `listeners/webhook-dispatch.listener.ts` — Central listener that subscribes to domain events (`UserCreatedEvent`, `InviteCreatedEvent`, `UserUpdatedEvent`, `UserDeletedEvent`, `OrgCreatedEvent`, subscription events, `UsageCollectedEvent`, `UserMessageCreatedEvent`, `SkillUsedEvent`, `ToolUsedEvent`, marketplace install events, `AddonActivatedEvent`, `AddonDeactivatedEvent`) and dispatches them as webhook HTTP calls via `SendWebhookUseCase`. `InviteCreatedEvent` becomes a distinct `user.invited` webhook so Startdeliver can create pending invitees without triggering sinks that handle real account creation, such as Brevo onboarding. Subscription webhooks are sequenced per organization so replacement cancellation reaches the receiver before replacement creation. The `chat.sent` webhook enriches the payload with the sender's email/name via `FindUserByIdUseCase`, and `user.created` enriches with a best-effort `orgName` via `FindOrgByIdUseCase` (AYC-445, consumed by the Brevo onboarding sink); both skip the lookup entirely when no webhook URL is configured.
...
- `infrastructure/services/webhook-delivery-sequencer.service.ts` — Maintains per-organization promise chains for subscription webhook delivery and drains outstanding chains during graceful shutdown.
- `domain/webhook-event.entity.ts` — Abstract `WebhookEvent` entity with UUID, event type enum, data payload, and timestamp. Concrete event types cover organization, user, subscription lifecycle, usage, chat, skill/integration product activity, and add-ons.
