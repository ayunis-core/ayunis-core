import type { AfterModelCallContext, Hook } from '@ayunis/agent-runtime';
...
/**
 * Builds the usage-metering hook: every physical provider attempt records its
 * reported tokens, including attempts interrupted before a runtime retry.
...
export class UsageHookFactory {
  ...
  create(params: { model: LanguageModel }): Hook {
    return {
      name: 'ayunis-usage',
      afterModelCall: (ctx) =>
        collectReportedUsage(this.inferenceUsageGuard, params.model, ctx),
...
function collectReportedUsage(
  guard: InferenceUsageGuard,
  model: LanguageModel,
  ctx: Pick<AfterModelCallContext, 'context' | 'iteration' | 'usage'>,
): void {
