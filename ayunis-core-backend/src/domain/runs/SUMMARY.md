...
Domain events are defined in `application/events/`: `RunExecutedEvent` (emitted when a run completes, includes userId and orgId), `InferenceCompletedEvent` (emitted after each inference call, includes userId, orgId, model, provider, streaming flag, duration, and optional error), `TokensConsumedEvent` (emitted with token usage data, includes userId, orgId, model, provider, input/output token counts), and `ToolUsedEvent` (emitted when a tool is invoked during a run, includes userId, orgId, and tool name). Each event class exposes a static `EVENT_NAME` string for type-safe listener registration.
