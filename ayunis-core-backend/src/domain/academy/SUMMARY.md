...
- `SubmitChapterQuizUseCase` grades against `requiredCorrect(total, threshold)`
  (ceil), enforces the drawn answer count, upserts `AcademyChapterProgress`
  (one row per user+chapter: `passedAt` refreshed on each pass, `lastScore`),
  and — when every `quizEnabled` chapter has a passing row — stamps the single
  per-user `AcademyCompletion.completedAt` snapshot. That snapshot is the anchor
...
Learner-facing (`@RequireAddon(AYUNIS_CORE_ACADEMY)`):

- `AcademyChaptersController` (`academy/chapters`) — list content (no quiz
  questions). `AcademyQuizController` (`academy`) — `GET chapters/:id/quiz`,
  `POST chapters/:id/quiz/submit`, `GET progress`.
