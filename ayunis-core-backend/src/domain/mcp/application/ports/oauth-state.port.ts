 ...
 * round-trips the {@link OAuthAuthorizationState} payload. The canonical
 * adapter uses a HS256 JWT via NestJS `JwtService`.
 */
export abstract class OAuthStatePort {
  /**
   ...
   */
  abstract sign(payload: OAuthAuthorizationState, ttlSeconds: number): string;
   ...
   */
  abstract verify(token: string): OAuthAuthorizationState;
}
