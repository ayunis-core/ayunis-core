...
describe('GetMonthlyCreditUsageForUserUseCase', () => {
  ...
  it('returns the credits the user has consumed in the current window', async () => {
    repository.getTotalMonthlyCreditUsageForUser.mockResolvedValue(1240);
  ...
  it('scopes the usage query to the given organization', async () => {
    repository.getTotalMonthlyCreditUsageForUser.mockResolvedValue(0);

    await useCase.execute(new GetMonthlyCreditUsageForUserQuery(orgId, userId));

    const [passedOrgId, passedUserId] =
      repository.getTotalMonthlyCreditUsageForUser.mock.calls[0];
    expect(passedOrgId).toBe(orgId);
  ...
  it('floors the window at the subscription anchor when it falls later in the month', async () => {
    ...
    await useCase.execute(
      new GetMonthlyCreditUsageForUserQuery(orgId, userId, anchor),
    );
