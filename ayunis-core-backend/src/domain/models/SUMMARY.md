...
- **ChatCompletionsController** (`presenters/http/openai/chat-completions.controller.ts`): OpenAI-compatible `POST /openai/v1/chat/completions` endpoint. Authentication is handled by the global `AuthGuard(['jwt', 'api-key'])` — the api-key Passport strategy validates `Authorization: Bearer ayk_live_...` callers and populates `request.user` with an `ActiveApiKey`. The controller resolves the requested model by name within the caller's org and routes the request to `GetInferenceUseCase` or `StreamInferenceUseCase` depending on the `stream` flag. Fair-use enforcement and trial-message accounting key off the principal (user UUID for browser callers, api-key UUID for api-key callers).
