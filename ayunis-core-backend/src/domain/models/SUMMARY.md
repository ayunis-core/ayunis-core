...
- **GetInferenceUseCase** (`application/use-cases/get-inference`): Routes a non-streaming inference request to the appropriate provider. Before dispatch it sanitizes replayed message history via `stripReplayedToolNulls` (`application/helpers/strip-replayed-tool-nulls.helper.ts`), which removes schema-disallowed nulls from persisted assistant tool calls (strict-mode leftovers) so they don't teach the next model turn to imitate the pattern; persisted entities are never mutated. Token-limited text responses remain errors by default, while callers that can represent token exhaustion as an expected completion may opt in even when no visible text was produced; token-limited tool calls are always rejected.
- **StreamInferenceUseCase** (`application/use-cases/stream-inference`): Routes and streams inference requests to the appropriate provider, applying the same `stripReplayedToolNulls` history sanitation before dispatch.
- **ResolveModelProviderUseCase** (`application/use-cases/resolve-model-provider`): Resolves a model to its credentialed `@ayunis/inference` provider so the agent runtime can stream through the existing host-owned provider registry without taking ownership of provider selection or credentials.
- **MapMessagesToInferenceUseCase** (`application/use-cases/map-messages-to-inference`): Converts backend thread messages into provider-agnostic inference messages for consumers outside the models module, applying the same `stripReplayedToolNulls` history sanitation before mapping.
