...
- **Ownership scope**: Every skill has exactly one owner. Personal skills have a `userId` and no workspace ID; workspace-owned skills have a `workspaceId` and no user ID. The scopes use separate lists and copies can retain origin/version metadata for explicit, optional refreshes without automatic synchronization.
