...
- **Ownership scope**: Every skill has exactly one owner. Personal skills have a `userId` and no workspace ID; workspace-owned skills have a `workspaceId` and no user ID. Copying a personal skill into a workspace creates a standalone resource without origin metadata or synchronization.
