...
- **Workspace activation**: Enabled workspace skills are candidates, not always-on instructions. `ActivateWorkspaceSkillByNameUseCase` resolves names through the authorized thread's workspace; `SkillAccessService.findActivatableSkill` delegates persisted thread/workspace matching to `SkillAuthorizationService` before checking workspace activation state and attaching resources. Personal/shared and workspace names have separate tool namespaces. Explicit quick actions use the same activation service. Disabling a skill prevents new activation; resources already attached to a conversation follow the regular skill lifecycle.
...
`SkillAuthorizationService` is the internal owner-aware policy for persisted skills. It keeps personal read/share and owner-only write rules local, while workspace read, write, and trusted execution delegate to exported workspace capabilities. Workspace denials become skill-not-found errors, and trusted activation derives its workspace from the authenticated persisted thread rather than tool or model input.
...
- **WorkspacesModule** — for owner-aware workspace read, write, and trusted-execution authorization capabilities
