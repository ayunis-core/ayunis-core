...
- **Ownership scope**: Personal skills have no workspace ID. Workspace-owned skills carry a workspace ID and remain separate from personal lists; `userId` records their creator rather than their durable owner. Copies can retain origin/version metadata for explicit, optional refreshes without automatic synchronization.
