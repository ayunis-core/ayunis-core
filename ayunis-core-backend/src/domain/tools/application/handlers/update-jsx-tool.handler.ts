...
@Injectable()
export class UpdateJsxToolHandler extends ToolExecutionHandler {
  private readonly logger = new Logger(UpdateJsxToolHandler.name);
  ...
  async execute(params: {
    ...
    this.logger.log('Executing update_jsx tool');
    ...
    try {
      ...
      return `JSX artifact updated successfully. Artifact ID: ${validatedInput.artifact_id}, version: ${version?.versionNumber}`;
    } catch (error) {
      ...
      this.logger.error('Failed to execute update_jsx tool', error);
      throw new ToolExecutionFailedError({
        toolName: tool.name,
