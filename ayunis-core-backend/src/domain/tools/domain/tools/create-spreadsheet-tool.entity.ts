...
const createSpreadsheetToolParameters = {
  ...
  properties: {
    ...
    rows: {
      ...
      description:
        'The data rows. Each row is an array of cells in column order. Use numbers for numeric values (not strings), and null for empty cells. A string starting with "=" is an Excel formula, e.g. "=SUM(B2:B10)"; headers are row 1, so the first data row is row 2.',
    },
  },
...
export class CreateSpreadsheetTool extends DisplayableTool {
  ...
  constructor() {
    super({
      ...
      description:
        'Create a new spreadsheet with tabular data that the user can view, edit in a grid, and export to Excel (.xlsx) or CSV. Use this when the user asks for a table, budget, list, or dataset they will want to edit or export. For a quick illustrative table inside an answer, prefer an inline markdown table instead. Prefer Excel formulas (cells starting with "=") over pre-computed totals so derived values stay correct when the user edits the data.',
