...
async function seedUser(
  ...
  // email is globally unique; skip (don't cross-wire) if it belongs elsewhere.
  const existing = await repo.findOne({ where: { email: user.email } });
  if (existing) {
    if (existing.orgId !== orgId) {
      console.warn(`⚠️  Skipping ${user.email}: belongs to another org.`);
      return null;
    }
...
async function seedUserUsage(
  ...
  const now = new Date();
  // Use the app's UTC month start so seeded rows land in the same window the
  // credit-limits screen sums (getEffectiveMonthStart), not the host's local month.
...
async function resolveUsers(
  ...
  for (const user of demoFixture.users) {
    const record = await seedUser(orgId, runner, user);
    if (record) usersByEmail.set(user.email, record);
  }
  // The existing minimal-seed admin is reused as a team member.
...
async function seedUsageForUsers(
  orgId: UUID,
  model: LanguageModelRecord,
  usersByEmail: Map<string, UserRecord>,
): Promise<void> {
  for (const user of demoFixture.users) {
    ...
    if (record) {
      await seedUserUsage(orgId, record.id, model, user.targetCredits);
    }
  }
}
...
async function seedDemo(runner: SeedRunner): Promise<void> {
  ...
  const usersByEmail = await resolveUsers(org.id, runner);
  await seedTeamsAndMemberships(org.id, usersByEmail);
  await seedUsageForUsers(org.id, model, usersByEmail);
