...
/** Standalone: requires seed:minimal to have created the usage org + model. */
...
async function seedDemo(runner: SeedRunner): Promise<void> {
  ...
  // Users (resolve admin from minimal too, for team membership).
  ...
  // Per-user usage so consumption-vs-limit is demoable.
  for (const user of demoFixture.users) {
