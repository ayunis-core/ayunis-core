...
export class SsoBrokerSessionCleanupTask {
  private readonly logger = new Logger(SsoBrokerSessionCleanupTask.name);

  constructor(private readonly sessions: SsoBrokerSessionsRepository) {}

  @Cron(CronExpression.EVERY_DAY_AT_5AM)
  async handleCleanup(): Promise<void> {
    try {
      const deleted = await this.sessions.deleteExpired(new Date());
      this.logger.log({ deleted }, 'Expired SSO broker sessions deleted');
    } catch (error) {
