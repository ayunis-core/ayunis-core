...
describe(PostgresOrgSsoConnectionsRepository.name, () => {
  ...
  it('normalizes identifiers before lookup', async () => {
  ...
  it('returns the surviving row after an identical concurrent insert', async () => {
    ...
    await expect(repository.save(anOrgSsoConnection())).resolves.toMatchObject({
      orgId: TEST_ORG_ID,
