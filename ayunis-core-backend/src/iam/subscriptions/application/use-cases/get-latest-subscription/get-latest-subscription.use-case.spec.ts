...
describe('GetLatestSubscriptionUseCase', () => {
  ...
  function createQuery(): GetLatestSubscriptionQuery {
    return new GetLatestSubscriptionQuery({ orgId, requestingUserId });
  }
  ...
  it('should use the requesting user id when loading open invites', async () => {
    ...
    expect(getInvitesByOrgUseCase.execute).toHaveBeenCalledWith(
