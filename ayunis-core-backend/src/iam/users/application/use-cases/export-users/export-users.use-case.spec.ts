...
import type { UsersExportRepository } from 'src/iam/users/application/ports/users-export.repository';
...
describe('ExportUsersUseCase', () => {
  ...
  it('should neutralize CSV formula injection in user-controlled fields', async () => {
    ...
    expect(dataRow).toBe(
      '"user-1","\'=cmd|calc","Lovelace","\'@evil@example.com","admin","\'+SUM(A1:A2)","\'-2+3","SEAT_BASED","01.07.2026"',
    );
  });
});
