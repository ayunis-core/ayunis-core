...
describe('SSO user policy persistence', () => {
  ...
  it('detects only accounts in other organizations with an exact matching domain', async () => {
    await expect(
      repository.hasUsersOutsideOrgWithEmailDomains(orgId, ['stadt.example']),
    ).resolves.toBe(false);
    ...
    await dataSource
      ...
      .update(
        { id: userId },
        { orgId: otherOrgId, email: 'staff@STADT.EXAMPLE' },
      );
    await expect(
      repository.hasUsersOutsideOrgWithEmailDomains(orgId, [' STADT.EXAMPLE ']),
    ).resolves.toBe(true);
    await expect(
      repository.hasUsersOutsideOrgWithEmailDomains(orgId, ['example']),
    ).resolves.toBe(false);
    await expect(
      repository.hasUsersOutsideOrgWithEmailDomains(orgId, [
        'school.stadt.example',
      ]),
    ).resolves.toBe(false);
    await expect(
      repository.hasUsersOutsideOrgWithEmailDomains(orgId, []),
