...
const TIER_KEYS: Record<ModelTier, TierKeyPair> = {
  [ModelTier.LOW]: {
...
export class SetFairUseLimitUseCase {
  ...
  async execute(command: SetFairUseLimitCommand): Promise<void> {
    // Look up via a widened record so TS doesn't assume the key is always
    // present — guards future non-HTTP callers that pass a bad enum value.
    const tierKeys = (TIER_KEYS as Record<string, TierKeyPair | undefined>)[
      command.tier
    ];
    if (!tierKeys) {
