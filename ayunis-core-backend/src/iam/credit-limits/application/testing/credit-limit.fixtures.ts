...
import type { CreditLimitRepository } from 'src/iam/credit-limits/application/ports/credit-limit.repository';
...
export const TEST_API_KEY_ID = '44444444-4444-4444-4444-444444444444' as UUID;
...
export function anApiKeyCreditLimit(
  overrides: { orgId?: UUID; apiKeyId?: UUID; monthlyCredits?: number } = {},
): ApiKeyCreditLimit {
  const {
    orgId = TEST_ORG_ID,
...
export function createMockCreditLimitRepository(): jest.Mocked<CreditLimitRepository> {
  return {
    ...
    findApiKeyLimits: jest.fn().mockResolvedValue([]),
