...
The guards are exported as plain providers; this module does **not** bind them globally. `IamModule` owns the global `APP_GUARD` bindings and their execution order (so the order between authentication and authorization guards is declared in one place rather than derived from NestJS module-scan order).
