...
@Command({ name: 'sso:enable', description: 'Enable SSO for an organization' })
export class EnableOrgSsoCliCommand extends CommandRunner {
  ...
  async run(_: string[], options: OrgSsoOptions): Promise<void> {
    const connection = await this.useCase.execute(
      new SetOrgSsoEnabledCommand(options.orgId, true),
    );
    writeSsoCommandResult('enabled', connection);
  }
}
