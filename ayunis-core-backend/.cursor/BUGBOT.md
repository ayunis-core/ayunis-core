...
- Imports outside the current directory must be absolute (`src/…`). Relative paths are only for same-directory siblings (`./thing`). If you see a `../` import anywhere in the diff, flag it — the absolute form is `src/` plus the path from `ayunis-core-backend/src`
