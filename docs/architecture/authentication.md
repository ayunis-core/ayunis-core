...
| `/accept-invite` | Invitation acceptance follows the invited organization's policy: password when allowed, otherwise organization-pinned SSO. Broker admission consumes the pending invite. |
...
Successful factor consumption and session issuance share one transaction.
Invalid-code and lockout outcomes commit their failed-attempt accounting first,
then the completion boundary throws the rejection. This uses one database
connection per request. Unexpected failures, including session-issuance errors,
still roll back successful factor consumption.
...
| Enabled, additive | Available | Allowed | Add verified domains without interruption; removing domains or changing the Zitadel organization requires disablement. |
| Enabled, SSO-only | Available | Blocked at every password admission/session boundary | Add verified domains while preserving broker identifiers; restore password login before changing the direct IdP. |
...
Transitioning from local password login to SSO-only, or extending the domains of
an SSO-only connection, checks for existing accounts with those exact domains in
another Core organization. A conflict rejects the operator's change without
modifying policy or revoking sessions. Repeating an already-enforced SSO-only
policy still validates the reviewed mapping but does not repeat the account
preflight or revoke sessions. This is a preflight safeguard, not a constraint on
future account creation. Adding domains to an active connection preserves its
existing domains, broker mapping and runtime switches; the conditional write
rejects stale changes with `SSO_CONNECTION_CHANGED`. The operator must reload,
review and retry; disabling SSO is not required for an additive-domain conflict.
Disabling SSO is also rejected while any member has no local password, even when
password login is otherwise allowed.
...
consume the read-only authentication-policy use case without reaching into SSO
persistence. Administrative policy mutations live in the full `SsoModule`,
which can depend on Users and Sessions without making the shared policy module
cyclic. `SessionsModule` deliberately imports neither Authentication nor
