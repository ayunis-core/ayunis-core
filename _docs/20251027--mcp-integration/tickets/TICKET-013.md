...
Implement the use case for enabling an MCP integration. This command sets the `enabled` flag to `true`, making the integration available for assignment to agents and use in conversations.

**Why**: Organization admins need to enable integrations that have been disabled or newly created in a disabled state.
...
5. Set enabled flag to true via domain entity method
...
**Files to create**:
- `src/domain/mcp/application/use-cases/enable-mcp-integration/enable-mcp-integration.command.ts`
- `src/domain/mcp/application/use-cases/enable-mcp-integration/enable-mcp-integration.use-case.ts`
...
export class EnableMcpIntegrationUseCase {
  ...
  async execute(command: EnableMcpIntegrationCommand): Promise<McpIntegration> {
    ...
    try {
      ...
      integration.enable(); // Domain entity method
      ...
      this.logger.error('Unexpected error enabling integration', { error });
...
- Test success path with disabled integration
- Test idempotency (already enabled)
