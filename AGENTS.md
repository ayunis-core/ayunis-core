...
export class SomeUseCase {
  ...
  async execute(command: SomeCommand): Promise<SomeResult> {
    ...
    try {
      ...
      this.logger.error("Unexpected error in use case", {
...
import {
  useItemsControllerCreate,
  useItemsControllerListQueryKey,
    ...
    mutation: {
      ...
      onSettled: () => {
        queryClient.invalidateQueries({
          queryKey: useItemsControllerListQueryKey(),
        });
        router.invalidate();
      },
    },
...
- Imported in `src/i18n.ts`

## Development Environment

- Go to backend folder and run `npm run docker:dev`
