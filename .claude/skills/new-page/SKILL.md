...
- A running slot is needed only for the final visual check. Use the slot the survey's runtime answer allowed; if it was "None", prove the page with a component test instead
  ...
  const mutation = useMyEntitiesControllerCreate({
    mutation: {
      ...
      onError: (error: unknown) => {
        ...
        showError(t(`errors.${code}`, { defaultValue: t('toast.createError') }));
      },
    },
  });
...
# src/shared/locales/de/<page-name>.json
# src/shared/locales/en/<page-name>.json
```
...
Register the namespace in `src/i18n.ts` (import both JSON files and add them to `resources`).
...
3. **Route tree**: `src/app/routeTree.gen.ts` is regenerated by the `TanStackRouterVite` plugin in `vite.config.js` whenever `vite` dev or `pnpm run build` runs. There is no separate CLI step.
...
- **Simple list page**: `src/pages/workspaces/` — paginated list with create dialog
- **Detail page with params**: `src/pages/workspace/` — single entity view with tabs and `$id` param
...
- **Data loading in route**: `src/app/routes/_authenticated/workspaces.index.tsx` — `loaderDeps` + loader with query prefetch
