...
describe('FilesystemSkillSource', () => {
  ...
  it('rejects duplicate identities and canonical-name changes', async () => {
    const root = await temporaryDirectory();
    const firstPath = await writeSkill(root, 'first', { name: 'same' });
    await writeSkill(root, 'second', { name: 'same' });
    const duplicateSource = new FilesystemSkillSource({ root });

    await expect(duplicateSource.list()).rejects.toThrow(
      /name must match its parent directory/i,
    );
