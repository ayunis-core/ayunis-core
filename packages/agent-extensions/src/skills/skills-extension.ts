...
interface SkillsRunState {
  ...
  readonly claimedToolNames: Set<string>;
...
const createActivationTool = (
  ...
  execute: async (input, { context, toolCallId }) => {
    ...
    return loadActivation(source, state, name, toolCallId);
  },
});
...
const loadActivation = async (
  ...
  try {
    const definition = await source.load(name);
    validateDefinition(definition, name);
    const toolNames = claimToolNames(state, definition.tools ?? []);
...
const claimToolNames = (
  ...
  const claimed = new Set(state.claimedToolNames);
  ...
  for (const name of toolNames) {
    state.claimedToolNames.add(name);
  }
  return toolNames;
};
...
const createSkillsHook = (): Hook => ({
  ...
  runStart: ({ context, tools }) => {
    context.set<SkillsRunState>(RUN_STATE, {
      activatedNames: new Set(),
      claimedToolNames: new Set(tools.map(({ name }) => name)),
...
const releaseToolNames = (
  ...
  for (const name of toolNames) {
    state.claimedToolNames.delete(name);
  }
};
