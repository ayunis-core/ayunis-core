...
export class ToolNameCodec {
  ...
  constructor(tools: readonly ToolSchema[]) {
    ...
    // legacy history (which stores such names) keeps matching.
  ...
  // A name absent from the map belongs to history: either the legacy
  // sanitized form of a declared tool (safe — pass it through so old rows
  // keep matching) or a tool detached mid-conversation (unsafe — sanitize
  // it, but never onto a declared tool's wire name, which would
  // misattribute the call).
